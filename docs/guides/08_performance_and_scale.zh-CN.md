...
- 临时路径按批次隔离为 `staging/<batch_id>/<host>/` 和 `staging_aux/<batch_id>/<host>/`。每条通路仍先完成全部暂存，再分组转换。`flatten_batch_bytes` 是目标，不可拆分的分片可能超过它。文件和分片元数据随其数量增长，转换前可能积累完整暂存数据。
- gzip 和 Arrow 两种路径都**默认保留暂存**。`keep_staging=False` 在转换成功后删除本轮分片，因此不能消除暂存磁盘峰值。磁盘预算应同时覆盖证据、暂存、Parquet 和临时工作文件；压缩比例取决于具体内容。
