...
- 同一次导入过程中，它还会发现并归一化：磁盘上的**计划任务**定义（一种持久化痕迹）、**IIS/nginx/Apache/Tomcat**
  ...
  MSSQL、Oracle 告警日志），以及 **Windows 注册表**配置单元（SYSTEM/SOFTWARE/SAM/
  SECURITY/DEFAULT，以及每个用户的 NTUSER.DAT/UsrClass.dat）。每种格式都是根据内容而非文件名判断的，因此被重命名或迁移过的证据文件同样能被正确识别。完整的十一张表全貌见[《2. 日志类型与模式》](02_log_types_and_schema.zh-CN.md)。
...
- **[2. 日志类型与模式](02_log_types_and_schema.zh-CN.md)** -- 十一张表各自存放什么，该看什么。
